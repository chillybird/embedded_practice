...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    // STA event
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) // 连接到wifi
    {
        esp_wifi_connect();
    }
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED) // wifi连接已断开，尝试重新连接
    {
        esp_wifi_connect();
        set_wifi_sta_status(WIFI_STA_MODE_INIT);
        ESP_LOGI(TAG, "retry to connect to the AP");
    }
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) // 已连接到wifi并成功获取IP地址
    {
