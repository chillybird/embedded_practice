...
const uint8_t line_width = 182;
const uint8_t max_line_num = 6;
...
void Paint_DrawString_CN(UWORD Xstart, UWORD Ystart, const char *pString, UWORD Color_Foreground, UWORD Color_Background)
{
    ...
    while (*(pstr + i) != '\0')
    {
        ...
        printf("line:%d x:%d y:%d w: %d code: %d size: %d \r\n", line_count, point_x, point_y, box_w, char_cn_un, size);
        ...
        for (uint8_t Page = 0; Page < page_len; Page++) // 0-127
        {
            for (uint8_t Column = 0; Column < colnum_len; Column++) // 0-1
            {
                ...
                {
                    point_x = Xstart + 2 * Column + (1 - index / 4) + (en_box_w + cn_box_w) + (en_data_num + cn_data_num) * 3 + offset_x;
                    point_y = Ystart + Page - offset_y - page_len + line_count * line_height; // line_height为一行的高度
                    if (((tmp >> index) & 0x0f) >= 1)
                    {
                        Paint_SetPixel(point_x, point_y, Color_Background);
                    }
                    else
                    {
                        Paint_SetPixel(point_x, point_y, Color_Foreground);
                    }
                }
            }
        }
...
uint8_t* ds_get_paint_canvas(void)
{
    return Paint._Image;
}
